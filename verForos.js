...
function cargarforos() {
    fetch('mostrar_foros.php')
        .then(response => response.json())
        .then(data => {
            const contenedorPublicaciones = document.getElementById('foros');
             if (!data.length) {
            contenedorPublicaciones.innerHTML = '<div class="post"><p style="text-align:center;">No hay foros</p></div>';
            return;
        }
            ...
            data.forEach(foros => {
                ...
                nuevaPublicacion.id = `post-(${foros.foro_id})`;

                nuevaPublicacion.innerHTML = `
              <div class="post-header">
                  <img src="${foros.imagen}" alt="Foto de usuario">
                  <div class="post-info">
                      <div class="post-info-name">
                          <p><strong  style="font-size:1.8rem">${foros.titulo}</strong> <span style="color:grey; font-size:1rem">Creado por: ${foros.nombre} ${foros.apellido}</span></p>
                          <span>${foros.fecha}</span>
              ...
              <div class="post-content">
                 ...
                 <a class="enlace-foro-btn" href="foro.php?id=${foros.foro_id}">Visitar foro</a>
              </div>
              `;
